...
/**
 * Unit tests verifying the mathematical consistency of the MCIncrementalMetric.
 * It ensures that the optimized warm-start incremental algorithm produces exactly
 * the same distance values as the full baseline MatchingClusterMetric across
 * various complex NNI move trajectories and rollbacks.
 */
public class MCIncrementalMetricTest {
    ...
    @BeforeEach
    void setUp() {
        // Initialize both the incremental metric and the full classic baseline metric
        incrementalMetric = new MCIncrementalMetric();
        classicMetric = new MatchingClusterMetric();

        // Use the 10-leaf rooted trees from the factory for a robust evaluation space
    ...
    void testInitialDistanceConsistency() throws Exception {
        ...
        // Assert: The initial incremental state must perfectly match the classic full calculation
        assertEquals(expectedClassicDist, incrementalMetric.getCurrentDistance(), DELTA,
                "Initial distance must be perfectly identical between incremental and classic metrics!");
    }
    ...
    void testSingleNniMoveAndUndoConsistency() {
        ...
        // We find these nodes to perform a precise matching NNI move.
        ...
        // Act: Apply the local NNI update incrementally
        ...
        assertEquals(0.0, distAfterMove, DELTA,
                "The target-matching NNI move should reduce the incremental distance to 0.0!");
        assertEquals(0.0, incrementalMetric.getCurrentDistance(), DELTA);
        ...
        assertEquals(initialDist, incrementalMetric.getCurrentDistance(), DELTA,
                "The rollback operation (undo) failed to restore the exact original distance!");
    }
    ...
    void testMultipleSequentialNniMovesMaintainStateConsistency() {
        ...
        // Define a chain of consecutive NNI steps (moving trajectory)
        NniMove move1 = new NniMove(n2, n3); // Corrects the tree (Distance drops to 0.0)
        NniMove move2 = new NniMove(n3, n4); // Diverges the tree structure
        NniMove move3 = new NniMove(n1, n2); // Further divergence

        // Act & Assert: Execute sequence forwards while tracking value updates
        double dist1 = incrementalMetric.applyNni(move1);
        assertEquals(0.0, dist1, DELTA);

        double dist2 = incrementalMetric.applyNni(move2);
        assertTrue(dist2 >= 0.0, "Distance should stay non-negative after structural divergence.");

        double dist3 = incrementalMetric.applyNni(move3);
        ...
        assertEquals(initialDist, incrementalMetric.getCurrentDistance(), DELTA,
                "After wiping the entire move history stack, the distance must equal the initial baseline!");
    }
    ...
    void testComplexBranchingTrajectoryWithNestedUndos() {
        ...
        // FIXED: Added missing declaration for node "1"
        Node n1 = TreeUtils.getNodeByName(baseTree, "1");
        Node n2 = TreeUtils.getNodeByName(baseTree, "2");
        Node n3 = TreeUtils.getNodeByName(baseTree, "3");
        Node n4 = TreeUtils.getNodeByName(baseTree, "4");
        ...
        incrementalMetric.undoNni(move3);
        assertEquals(dist2, incrementalMetric.getCurrentDistance(), DELTA,
                "Backing out of a speculative dead end branch corrupted the metric state!");

        // Phase 3: Take an alternate evolutionary path (1 new step forward)
        double dist4 = incrementalMetric.applyNni(move4);
        ...
        incrementalMetric.undoNni(move1);
        assertEquals(initialDist, incrementalMetric.getCurrentDistance(), DELTA,
                "Full trajectory recovery failed to clean up historical assignment variables!");
    }
}