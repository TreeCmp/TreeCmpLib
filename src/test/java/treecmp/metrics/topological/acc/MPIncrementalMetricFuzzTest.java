 ...
 * and unpredictable rollbacks (undo) to ensure the incremental optimization
 * NEVER drifts from the pure baseline MatchingPairMetric.
 */
public class MatchingPairIncrementalMetricFuzzTest {
    ...
    void testFuzzRandomTrajectoriesAndRollbacks() {
        ...
        // Stałe ziarno dla pełnej powtarzalności testu
        Random random = new Random(42);
        Stack<Tree> treeHistory = new Stack<>();

        for (int i = 0; i < FUZZ_ITERATIONS; i++) {
            // 1. Zbieramy wszystkie matematycznie w 100% poprawne ruchy NNI
            ...
            // Jeśli drzewo degeneruje do braku wewn. węzłów (mało prawdopodobne, ale bezpieczne)
            ...
            // Zapisujemy stan przed ruchem (na wypadek Undo)
            treeHistory.push(currentTestTree);

            // 3. Budujemy "Oczekiwaną" topologię za pomocą oryginalnej biblioteki PAL i liczymy baseline
            ...
            // 4. Aplikujemy ten sam ruch w naszej nowej, O(N^2) hybrydowej metryce
            double actualDistance = incrementalMetric.applyNni(move);

            // 5. EGZEKUCJA ASERCJI: Odległość musi być perfekcyjnie spójna
            assertEquals(expectedDistance, actualDistance, DELTA,
                    "Fuzz mismatch at iteration " + i + "! Incremental logic drifted from baseline.");
            ...
            if (random.nextDouble() < 0.3) {
                ...
                currentTestTree = treeHistory.pop(); // Cofamy drzewo testowe

                // Weryfikujemy, czy wycofanie przywróciło macierz LAP do poprawnego stanu
                ...
                assertEquals(expectedUndoDist, incrementalMetric.getCurrentDistance(), DELTA,
                        "Undo mismatch at iteration " + i + "! History stack corruption detected.");
            }
        }
    }
