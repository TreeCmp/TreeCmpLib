...
@DisplayName("Weryfikacja poprawności 1-krokowego Ecr2IncrementalHeuristic vs klasyczne SubtreeEcr2Utils")
public class Ecr2IncrementalCorrectnessTest {

    private static final double EPSILON = 1e-9;

    /**
     * Generator parametrów dla 2-sECR:
     * Otoczenie o wielkości O(N^2), więc testujemy rozmiary od 10 do 80.
     */
    private static Stream<Arguments> provideTestParameters() {
        String[] metrics = {"RF", "RFC", "MS", "MC", "MP", "M3"};
        int[] treeSizes = {10, 20, 50, 80};
        long[][] seedPairs = {
                {12345L, 67890L},
                {42L, 999L},
                {2026L, 1337L}
    ...
    @DisplayName("Porównanie minimalnej odległości w otoczeniu 2-sECR (Incr vs Classic)")
    void testSingleStepBestNeighborMatch(String metricName, int treeSize, long seed1, long seed2) throws Exception {
        ...
        TreeNeighborhoodUtils classicUtils = new SubtreeEcr2Utils(!isRooted);

        // 1. Obliczenie wyniku inkrementalnego
        double distIncr = incrementalMetric.evaluateSingleStep(t1ForIncr, t2);

        // 2. Obliczenie wyniku klasycznego (przegląd całego wygenerowanego otoczenia w locie)
        ...
        classicUtils.forEachNeighbour(t1, n -> {
            ...
            try {
                ...
                throw new RuntimeException("Błąd podczas ewaluacji odległości w teście 2-sECR", e);
            }
        });

        // Zamiast assertTrue(neighbors.length > 0) sprawdzamy nasz wewnętrzny licznik
        assertTrue(neighborCount[0] > 0, "Otoczenie 2-sECR nie powinno być puste");
        ...
        assertEquals(
                ...
                EPSILON,
                String.format(
                        "Niezgodność w 2-sECR (%s) dla n=%d (seeds=%d/%d)! Classic=%.6f vs Incr=%.6f",
