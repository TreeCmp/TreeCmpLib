...
def main():
  ...
  print(
      f"Starting verification of {total_files} trajectory certificate"
      " files..."
  )
  ...
  for idx, f in enumerate(files, 1):
    ...
    runs = parse_vnd_runs(str(f), shared_tns)
    ...
    for r_idx, steps in enumerate(runs, 1):
      ...
      results.append(
          TrajectoryResult(
              f.name, nni_ok, conv, f_dist, n_steps, metric, variant
          )
      )
  ...
  converged = sum(
      1 for r in results if r.is_valid_nni_chain and r.is_full_convergence
  )

  print("=" * 80)
  print(
      "TRAJECTORY STEP VALIDATION (RF=2):"
      f" {valid_chains}/{total} ({valid_chains/total*100:.1f}%)"
  )
  ...
  if show_summary:
    ...
    for (m, v), items in sorted(groups.items()):
      ...
      count = len(items)
      succ_dists = [x.final_distance for x in items if x.is_full_convergence]
      avg_d = (
          f"{sum(succ_dists)/len(succ_dists):.4f}" if succ_dists else "None(Inf)"
      )
      print(f"{m:<10} | {v:<30} | {succ:>3}/{count:<6} | {avg_d:<12} | {count}")
    print("-" * 80)
